...
Item {
    ...
    function openPopup() {
        ...
        citySelectorPopup.openWithSelected(cityItems());
    }

    function cityItems() {
        var items = [];
        for (var i = 0; i < citySelector.selectionModel.count; i++)
            items.push(selectionModel.get(i));
        return items;
    }

    function reset(incomingJson) {
        selectionModel.clear();

        for (var i of incomingJson) {
            citySelector.selectionModel.append(i);
        }
    }
